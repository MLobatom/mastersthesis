...
Firstly, to set the environment for the analysis we run in Ubunto the script called "setting_up_conda_env.sh". Then, in the folder with the fastq files we did the quality control by running "fastqc.sh".
...
Afterwards, we used "kallisto_algnments.sh" to make the alignments to reference genomes, we must introduce firstly the name of the species to whose genome we want to align the fastq files and then the condition (for example 3dpi_269) due to the defined format of the file names. 
